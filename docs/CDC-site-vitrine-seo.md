...
| `Content-Security-Policy` | Restrictive : `default-src 'self'`. Deux exceptions : Google Identity Services (`accounts.google.com/gsi/`) pour la sauvegarde Drive - `script-src`, `style-src`, `connect-src`, `frame-src` ; et `'wasm-unsafe-eval' 'unsafe-eval'` dans `script-src`, requis par yoga-layout (moteur de mise en page de `@react-pdf/renderer`) sans quoi la génération des PDF échoue à l'étape E4 - le second pour Safari < 16.4 (iPadOS 15 / 16.0-16.3), qui ignore `'wasm-unsafe-eval'` et refuse alors d'instancier le WebAssembly ; à retirer quand ce parc aura disparu. À valider contre le besoin réel de l'app (blobs PDF, `data:` images, WASM) avant mise en production |
