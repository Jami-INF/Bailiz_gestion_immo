...
export function BailRapidePage() {
  ...
  /**
   * Saisie libre du logement : formulaire complet affiché à la demande, pour un
   * bien utilisé sur ce seul bail sans être enregistré. Masqué par défaut - il
   * reprend les champs de la modale « Créer un logement ». `null` = pas encore
   * de choix, on retombe alors sur le défaut calculé plus bas.
   */
  const [saisieLibreChoisie, setSaisieLibre] = useState<boolean | null>(null);
  /**
  ...
  // Par défaut, seule l'édition d'un bail écrit sur un logement jamais enregistré
  // ouvre la saisie libre : ses données doivent rester visibles et modifiables.
  const saisieLibre = saisieLibreChoisie ?? (edition && !saisie.bienId);
      ...
      <div className="xl:grid xl:grid-cols-[minmax(0,1fr)_minmax(460px,50%)] xl:items-start xl:gap-6">
        ...
        <div className="space-y-4">
          ...
          <Section titre="Logement">
            {/*
              Un seul formulaire de logement visible à la fois : le bouton
              « Créer un logement » ouvre une modale aux champs identiques à la
              saisie libre, les afficher ensemble donnait l'impression de deux
              formulaires d'ajout pour la même chose. La saisie libre - un bien
              utilisé pour ce bail sans être enregistré - reste accessible, mais
              à la demande.
            */}
            {biens.length > 0 && (
              <div className="min-w-0">
                <Field label="Bien" hint="Choisissez un bien enregistré (toutes ses infos sont utilisées, loyer et charges compris), créez-le, ou saisissez-le pour ce seul bail.">
                  <Select
                    value={saisie.bienId ?? ''}
                    onChange={(e) => {
                      const id = e.target.value || undefined;
                      choisirBien(id);
                      if (id) setSaisieLibre(false);
                    }}
                  >
            ...
            {bienChoisi ? (
              ...
              <div className="flex flex-col items-start gap-3 rounded-lg border border-dashed border-accent-300 p-4 text-sm text-accent-600 sm:flex-row sm:items-center sm:justify-between">
                <span>
                  {biens.length === 0
                    ...
                    : 'Aucun bien sélectionné pour ce bail.'}
                </span>
              ...
              <>
                <div className="flex flex-col items-start gap-2 rounded-lg bg-accent-50 p-3 text-xs text-accent-600 sm:flex-row sm:items-center sm:justify-between">
                  <span>Logement saisi pour ce seul bail, sans être enregistré.</span>
                  <button
                    type="button"
                    onClick={() => setSaisieLibre(false)}
                    className="font-medium text-accent-700 underline"
                  >
                    Utiliser un bien enregistré à la place
                  </button>
                </div>
                <div className="grid gap-4 sm:grid-cols-2">
