...
export function BienFormPage() {
  ...
  const enregistrer = async () => {
    if (!validerEtape()) return;
    await db.biens.put({ ...bien, updatedAt: nowISO() });
    toast('success', id ? 'Bien mis à jour.' : 'Bien créé.');
    navigate(`/biens/${bien.id}`);
  };

  const listeToText = (l: string[]) => l.join('\n');
  ...
  return (
    <div>
      ...
      <Card className="space-y-4">
        {etape === 0 && (
          <>
            <Field label="Nom du bien" required error={erreurs.nom} hint='Ex. "T2 Chamalières"'>
              <Input value={bien.nom} onChange={(e) => maj({ nom: e.target.value })} />
            </Field>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              ...
              <Field label="Étage">
                <Input value={bien.etage ?? ''} onChange={(e) => maj({ etage: e.target.value })} />
              </Field>
              <Field label="Bâtiment">
                <Input value={bien.batiment ?? ''} onChange={(e) => maj({ batiment: e.target.value })} />
              </Field>
        ...
        {etape === 1 && (
          <>
            <div className="grid grid-cols-2 gap-4">
              <Field label="Surface habitable loi Boutin (m²)" required error={erreurs.surfaceBoutin}>
                <Input
                  type="number"
                  step="0.01"
                  min="0"
              ...
              <Field label="Nombre de pièces principales" required error={erreurs.nbPieces}>
                <Input
                  type="number"
                  min="1"
                  value={bien.nbPieces || ''}
            ...
            <Field label="Équipements privatifs" hint="Un par ligne (ex. cuisine équipée, interphone…)">
              <Textarea
                value={listeToText(bien.equipementsPrivatifs)}
                onChange={(e) => maj({ equipementsPrivatifs: textToListe(e.target.value) })}
              />
            </Field>
            <Field label="Parties et équipements communs" hint="Un par ligne (ex. ascenseur, local vélos…)">
              <Textarea
                value={listeToText(bien.partiesCommunes)}
                onChange={(e) => maj({ partiesCommunes: textToListe(e.target.value) })}
              />
            </Field>
            <Field label="Annexes (cave, parking…)" hint="Une par ligne, au format « type : description » (ex. cave : n°12 sous-sol)">
              <Textarea
                value={bien.annexes.map((a) => `${a.type} : ${a.description}`).join('\n')}
                onChange={(e) =>
                  maj({
                    annexes: textToListe(e.target.value).map((ligne) => {
